      ...
      if (Object.keys(depMap).every((m) => typeof depMap[m] === 'string')) {
        ...
        Object.keys(depMap).forEach((k) =>
          updateDepMapUrls(
            depMap,
            k,
            evalDep(depMap[k], rootUrl, name, version)
          )
        );
...
function evalDep(
  ...
  return `${rootUrl}/modules/${name}/${version}/${dependency}`;
}
...
export function extractPiletMetadata(
  ...
  if (checkV1.test(main)) {
    ...
    return {
      ...
      link: `${rootUrl}/modules/${name}/${version}/${file}`,
      license,
    };
...
export function getPiletDefinition(
